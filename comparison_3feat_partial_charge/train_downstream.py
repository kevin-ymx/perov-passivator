"""
Downstream binding-energy training (3-feature: atomic_num + tetrahedral_chirality + partial_charge).

Same training pipeline as the main LBPP train_downstream.py (weighted MSE, val-MAE
checkpointing, CLI, eval splits). Graph node features: 3-D (atomic_num, chirality, partial_charge).
...
def load_adsorption_data(
    ...
    node_feature_names = ['atomic_num', 'chirality', 'partial_charge']
...
def load_extra_smiles_csv(
    ...
    Graphs use 3 node features: atomic_num, chirality, partial_charge (same as MolecularGraphWithBinding.mol_to_graph).
