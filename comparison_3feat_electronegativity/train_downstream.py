"""
Downstream binding-energy training (3-feature: atomic_num + tetrahedral_chirality + electronegativity).

Same training pipeline as the main LBPP train_downstream.py (weighted MSE, val-MAE
checkpointing, CLI, eval splits). Graph node features: 3-D (atomic_num, chirality, electronegativity).
    ...
    def mol_to_graph(cls, mol: Chem.Mol, binding_atom_indices: List[int] = None) -> Data:
        ...
        for atom in mol.GetAtoms():
            ...
            node_features.append([
                ...
                float(cls.get_electronegativity(an)),
            ])
...
def load_adsorption_data(
    ...
    node_feature_names = ['atomic_num', 'chirality', 'electronegativity']
...
def load_extra_smiles_csv(
    ...
    Graphs use 3 node features: atomic_num, chirality, electronegativity (same as MolecularGraphWithBinding.mol_to_graph).
